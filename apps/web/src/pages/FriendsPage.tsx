...
export function FriendsPage() {
  ...
  return (
    <div className="page friends-page">
      ...
      <section><div className="section-heading"><span>ALL FRIENDS</span><b>{friends.length}</b></div><div className="friend-grid">{friends.map((friend) => <article className="friend-card glass-panel" key={friend.id}><div className="friend-card-top"><Avatar user={friend} size="xl" showStatus /><span className={`status-chip ${friend.status}`}>{friend.status}</span></div><h3>{friend.username}</h3><p>{friend.bio || 'In your NOVA orbit'}</p><div className="friend-card-actions"><Link to={`/app/chats/${friend.id}`}><MessageCircle />{t('friends.message')}</Link><button disabled={removing === friend.friendshipId} onClick={() => setRemoveTarget({ friendshipId: friend.friendshipId, username: friend.username })} aria-label={`Remove ${friend.username}`}><Trash2 /></button></div></article>)}</div></section>
