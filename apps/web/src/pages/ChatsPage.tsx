    ...
    <div className="chats-layout">
      <aside className={`conversation-pane ${hasActive ? 'mobile-hidden' : ''}`}>
        ...
        <div className="conversation-scroll">
          {filter === 'all' && filteredGroups.map((group) => <Link className={group.id === groupId ? 'active' : ''} to={`/app/groups/${group.id}`} key={group.id}><span className="group-call-avatar"><UsersRound /></span><span><strong>{group.name}</strong><small>{group.members.length} أعضاء</small></span></Link>)}
          {filtered.map((friend) => <Link className={friend.id === userId ? 'active' : ''} to={`/app/chats/${friend.id}`} key={friend.id}><Avatar user={friend} size="md" showStatus /><span><strong>{friend.username}</strong><small>{friend.lastMessage ?? 'Start a conversation'}</small></span><time>{friend.lastMessageAt ? new Date(friend.lastMessageAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : ''}{friend.unread ? <b>{friend.unread}</b> : null}</time></Link>)}
        </div>
